...
instance.prototype.action = function(action) {
	...
	var login = '<setup version="1" > <username>' + self.config.user + '</username> <password>' + MD5(self.config.pass) + '</password> <needack>Yes</needack> </setup> ';
	...
	switch(action.action) {
		...
		case 'user_preset_action':
			cmd = login + '<' + action.options.action + '_preset id="' + action.options.id + '" needack="Yes"/>';
			break;

		case 'layout_action':
			cmd = login + '<' + action.options.action + '_layout id="' + action.options.id + '" needack="Yes"/>';
			break;	
		...
		case 'reboot':
			cmd = login + '<reboot needack="Yes"/>';
			break;

		case 'wakeup':
			cmd = login + '<wake_up needack="Yes"/>';
			break;

		case 'standby':
			cmd = login + '<standby needack="Yes"/>';
			break;
	}
